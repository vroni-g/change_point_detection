...
rec_mosum <- test_CPD_efp(n, 'Rec-MOSUM', trend, h = 0.01) # tried different window sizes from 0.01 to 0.3 but always FPR = 0
...
fpr_LM <- sum(p_LM<0.05)/n # is 0
cat(paste('False positive rate for supLM is: ', fpr_LM))
fpr_maxmo <- sum(p_maxmo<0.05)/n # is 0
